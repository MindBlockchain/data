...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://mindblockchain.org - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/mindblockchain.org)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://mindblockchain.org'}
          color={text === 'https://mindblockchain.org' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
